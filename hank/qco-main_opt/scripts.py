...
def get_random_SU(n: int, order: int | None = None):
    ...
    Args:
        ...
        order (int | None): if given, returns a random conjugate of a diagonal matrix
            whose entries are order-th roots of unity with product 1 (i.e. in SU(n)).
    ...
    if order is not None:
        ...
        # Build an n-entry diagonal in SU(n): first n-1 entries are order-th roots
        # of unity, the last is chosen to force det = 1.
        phases = [np.exp(1j * 2 * np.pi * k * j / order) for j in range(n - 1)]
        phases.append(np.conj(np.prod(phases)))   # product of all phases = 1
...
def get_gates_from_file(fileName):
    ...
    with open(fileName, 'r') as f:
        lines = f.readlines()
        n = int(lines[0].strip())   # was lines[0][0] — broke for d >= 10
...
def _parse_gate_txt(path: str, d: int) -> np.ndarray:
    ...
    Accepts two formats:
      - npy_to_qco.py format: first line is the integer dimension, then d rows.
      - Raw format: exactly d lines of d space-separated complex numbers.
    Lines beginning with '#' are treated as comments and ignored.
    """
    with open(path) as f:
        lines = [l.strip() for l in f if l.strip() and not l.startswith('#')]

    # Skip a leading dimension header if present
    try:
        if int(lines[0]) == d:
            lines = lines[1:]
    ...
    if len(mat) != d or any(len(row) != d for row in mat):
        raise ValueError(
            f"Expected a {d}x{d} matrix in '{path}', "
            f"got {len(mat)} rows."
        )
    return np.array(mat)
...
def sample_norms(sample_size, n_of_generators, d=2, v='0.0.0',
    ...
    **kwargs):
    ...
    if from_file:
        ...
        # Derive a clean group identifier from the filename, e.g.
        # "data/clifford.txt" -> "clifford", "BO.npy" -> "BO"
    ...
    # --- Build the fixed extension gate and record its type/value in options ---
    #
    # Three modes, in priority order:
    #   fixed_gate_matrix  : arbitrary pre-specified NxN unitary (from .npy or .txt)
    #   fixed_gate_angles  : diagonal SU(d) gate; provide d-1 phase angles (multiples
    #                        of pi) as a comma-separated string, e.g. "0.3,0.7"
    #   fixed_gate_angle   : legacy SU(2) single-angle diagonal gate (kept for
    #                        backward compatibility)
    #   (none)             : Haar-random SU(d) gates
    #
    # The keys 'exttype' and 'extval' are added to options so they appear in the
    # output filename, making every run self-describing.

    fixed_gate = None

    if fixed_gate_matrix is not None:
        # Load an arbitrary pre-specified SU(d) matrix from a .npy or text file,
        # then normalise to SU(d) by dividing out the d-th root of the determinant.
        ...
        # General diagonal SU(d) gate: d-1 phase angles given as multiples of pi.
        # The d-th phase is chosen automatically to enforce det = 1.
        ...
        phases.append(np.conj(np.prod(phases)))   # enforces det = 1
        fixed_gate = np.asmatrix(np.diag(phases))
        ext_type = 'angles'
        ext_val  = angle_vals   # list rendered as "v1_v2_..." by myStr in get_file_name
        options  = {**options, 'exttype': ext_type, 'extval': ext_val}

    elif fixed_gate_angle is not None:
        # Legacy single-angle path: diag(e^{i*a*pi}, 1, ..., 1) normalised to SU(d).
        ...
        # Pure Haar-random: record in filename so runs are self-describing.
