...
class uRepresentation:
    ...
    def get_representation(self, a):
        ...
        """        
        # Accumulate COO data from each basis element scaled by a[i][j],
        # then build a single csr_matrix at the end — avoids repeated
        # sparsity-structure modification that triggers SparseEfficiencyWarning.
        rows, cols, vals = [], [], []
        ...
        return sparse.csr_matrix(
            (vals, (rows, cols)), shape=(self.dim, self.dim), dtype=complex
        )
...
class GroupRepresentation:
    ...
    def __call__(self, U):
        with warnings.catch_warnings():
            ...
            warnings.filterwarnings(
                "ignore",
                message="Changing the sparsity structure of a csc_matrix is expensive. "\
                "lil and dok are more efficient."
            )
            pi_log = self.pi(logm(U))
            return sparse_expm(pi_log.tocsc()) if sparse.issparse(pi_log) else expm(pi_log)
