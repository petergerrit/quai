...
Clifford$+P(2\pi/9)$, $P(\pi/9)$, $P(\pi/18)$ & \texttt{Z[$\omega_{18}$, 1/2]} & \textsc{partial (indirect)}: no direct stabilizer-code distillation (Anderson--Jochym-O'Connor~\cite{Anderson:2014}); programmable-MSD protocols distil $|\psi(\theta)\rangle$ for arbitrary $\theta$ on a Clifford$+T$ substrate at overhead comparable to Ross--Selinger for $\theta\!\sim\!\pi/4$ & \cite{DuclosCianciPoulin:2015,CampbellOGorman:2016} \\
Clifford$+P(2\pi/5)$                & \texttt{Z[$\omega_{10}$]}           & \textsc{partial (indirect)}: direct stabilizer-code distillation excluded~\cite{Anderson:2014}; programmable-MSD on Clifford$+T$ substrate applies to arbitrary $\theta$ & \cite{DuclosCianciPoulin:2015,CampbellOGorman:2016} \\
