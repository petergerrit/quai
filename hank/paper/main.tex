...
The $6.8\times$ advantage should therefore be read as a \emph{primitive-gate-set compilation depth} result, not a certified fault-tolerant resource claim. Direct stabilizer-code distillation of $|k\pi/9\rangle$ states is excluded by Anderson--Jochym-O'Connor~\cite{Anderson:2014}, but two separate fault-tolerant routes bypass that exclusion. The first is programmable magic-state distillation~\cite{DuclosCianciPoulin:2015,CampbellOGorman:2016,CampbellHoward:2017} built on a Clifford$+T$ substrate, which distils $|\psi(\theta)\rangle$ for operator-specified $\theta$. The second is non-stabilizer code-switching: the AJOC no-go applies to \emph{stabilizer} codes only, and the permutation-invariant family of Kubischta--Teixeira hosts transversal $Z(\pi g/b)$ for rational $g/b$, which Ouyang--Jing--Brennen~\cite{OuyangJingBrennen:2024} compose with a stabilizer Clifford code via a measurement-free switch. At $\theta = 2\pi/9 \approx 40^\circ$ the programmable-MSD overhead is comparable to Ross--Selinger synthesis in Clifford$+T$, so the $6.8\times$ BFS-depth advantage does not translate directly into a $6.8\times$ reduction in fault-tolerant resource cost. The advantage is most cleanly realised when $P(2\pi/9)$ is available as a native primitive---e.g., in a qutrit-hosted register with code-switching, or via a hypothetical nonadditive code distilling $|\omega_9\rangle$ at unit cost. The \texttt{bfs\_estimate} tag in \cref{tab:lamm_sigma36} signals this caveat; the advantage survives any plausible tightening on the canonical side but is contingent on the broader fault-tolerance stack.
...
\paragraph*{$\Sigma$-series $+\,\HV{z}{\gamma}{\varepsilon}$ at $d=3$.} Partial, with data and distillation codes separable. The Howard--Vala family is the natural qutrit $T$-gate, and its magic state $|\omega_9\rangle$ is distilled directly by the qutrit triorthogonal family $[[9m-k,k,2]]_3$~\cite{Campbell:2012jna} with yield $\gamma \approx 1.51$; the ternary Golay $[[11,1,5]]_3$ code distils the ``strange'' qutrit state. Those \emph{distillation} codes are in hand. What remains open is a complementary \emph{data} code that hosts an entire $\Sigma$-series group transversally so its base-group operations are cheap. No such code is catalogued today, but two recent constructions point toward existence: the Denys--Leverrier covariant-encoding framework~\cite{DenysLeverrier:2024} realises qubit binary tetrahedral $2T$ transversally on $[[5,1,3]]$ and binary octahedral $2O$ on Steane, and the 2T-qutrit bosonic encoding~\cite{DenysLeverrier:2023} realises the same $2T$ transversally on a two-mode qutrit. Extending either line of work to the qutrit $\Sigma(m\!\times\!3)$ series---a specific open case inside the Kubischta--Teixeira twisted unitary $t$-group programme~\cite{KubischtaTeixeira:2024}---would supply the missing $C_\mathrm{data}$ and complete the stack, since $C_\mathrm{distil}$ is already in place. Until then, the fault-tolerant implementation of $\Sigma+\HV{z}{\gamma}{\varepsilon}$ proceeds via magic-state injection of $|\omega_9\rangle$ from a generic qutrit Clifford data code; this is standard fault-tolerance modulo the open transversal-pairing question.
...
Clifford$+P(2\pi/9)$, $P(\pi/9)$, $P(\pi/18)$ & \texttt{Z[$\omega_{18}$, 1/2]} & \textsc{partial (indirect)}: no direct stabilizer-code distillation (Anderson--Jochym-O'Connor~\cite{Anderson:2014}); programmable-MSD distils $|\psi(\theta)\rangle$ on a Clifford$+T$ substrate; PI code-switching~\cite{OuyangJingBrennen:2024} is a non-stabilizer alternative & \cite{DuclosCianciPoulin:2015,CampbellOGorman:2016,CampbellHoward:2017,OuyangJingBrennen:2024} \\
...
We have introduced \SWIFTbot, a paper-scale pipeline that systematically ranks qudit gate sets (base group plus extension) by the quantum circuit overhead framework of~\cite{Slowik:2025qco}, cross-validated it bit-identically against the $d=2$ reference data, and deployed it to catalogue the $d=3$ finite-subgroup-plus-extension landscape at Tier-1 resolution. Three findings stand out. First, at $d=3$ and $t=5$ the Haar-random extension on $\Sigmathree{360}$ reaches $\QT/\Popt \approx 1.32$---setting a numerically concrete target for a $d=3$ super-golden construction and identifying $\Sigmathree{360}$ as the best general-purpose base group in our registry. Second, and apparently in tension with the first, the non-canonical extension $\mathrm{Clifford}+P(2\pi/9)$ beats canonical $\mathrm{Clifford}+T$ by $6.8\times$ on the DSA primitive-gate target family at $d=2$, despite being $\sim\!8\times$ worse on $\QT$; this is not a paradox but a design lesson: paper-scale qudit fault-tolerance design benefits from reporting \emph{both} a Haar-universal $\QT$ ranking and a target-family-specific mean-depth ranking. Third, a panel-wide audit of the QEC-code and magic-state distillation catalogues shows that only canonical $\mathrm{Clifford}+T$ at $d=2$ has an end-to-end certified fault-tolerant stack in our evaluation set; each remaining winner is covered partially by current theory, with concrete algebraic-compilation or code-pairing problems standing between it and full fault-tolerance.

Three follow-up directions are pressing. The Tier-2 $d=3$ run at $t=10$ and the $d=2$ \texttt{BI} super-golden campaign with $\sim\!10^4$-sample statistics are in execution at the time of writing and will supersede the numbers quoted here in the final version. Beyond Tier-2, a $d=4$ pilot on the Fairbairn subgroups~\cite{Fairbairn:1964}, a Ross--Selinger-style normal form for the cyclotomic ring $\mathbb{Z}[\omega_{18},\tfrac{1}{2}]$ that would certify the DSA-family $T$-counts of \cref{tab:lamm_sigma36}, and an explicit $\Sigma$-series $\leftrightarrow$ data-code transversal pairing in the style of Denys--Leverrier's covariant encoding~\cite{DenysLeverrier:2024} or the Kubischta--Teixeira twisted unitary $t$-group framework~\cite{KubischtaTeixeira:2024} are the specific open problems identified in \cref{sec:discussion_open}. Each is sharp enough to settle and, if solved, would upgrade one of the partial fault-tolerance entries in \cref{tab:distillation} to a complete stack. The recent permutation-invariant code-switching route of~\cite{OuyangJingBrennen:2024} provides a concrete template for the last of these at $d=2$; a qutrit analogue is open.
