...
def random_diagonal_sun_batch(n: int, n_samples: int) -> np.ndarray:
    """
    Generate `n_samples` random diagonal SU(N) matrices.

    A diagonal unitary matrix has the form diag(e^{iθ_1}, ..., e^{iθ_n}).
    The SU(N) constraint det = 1 requires sum(θ_k) = 0 (mod 2π).

    Construction: draw (n-1) angles θ_1…θ_{n-1} uniformly from [0, 2π),
    then set
        θ_n = -(θ_1 + ... + θ_{n-1})
    so that sum(θ_k) = 0 exactly. This gives uniform measure on the
    (n-1)-torus T^{n-1} ≅ diagonal SU(N).

    Returns D : np.ndarray, shape (n_samples, n, n), dtype complex128,
    with zeros everywhere except the diagonal.
    """
    # (n_samples, n-1) independent uniform phases
    thetas = np.random.uniform(0.0, 2 * np.pi, size=(n_samples, n - 1))
    # Last angle enforces sum = 0
    last = -np.sum(thetas, axis=1, keepdims=True)            # (n_samples, 1)
    all_thetas = np.concatenate([thetas, last], axis=1)      # (n_samples, n)
    phases = np.exp(1j * all_thetas)                         # (n_samples, n)
    # Build diagonal matrices: D[k, i, j] = phases[k, i] * δ_{ij}
    D = np.zeros((n_samples, n, n), dtype=complex)
    D[:, np.arange(n), np.arange(n)] = phases
    return D
...
def sample_un_voronoi(clifford, n_samples=1000):
    """Voronoi cell statistics for diagonal SU(N). See sample_un_voronoi_fast for the
    accelerated path used in the main loop."""
    n = clifford.shape[-1]
    n_clifford = len(clifford)
    samples   = random_diagonal_sun_batch(n, n_samples)
...
out_path = os.path.join("diag_data", f"data_{stem}")
...
# Diagonal SU(N) samples are sparse (only n non-zeros per matrix), but we
# still materialise full n×n arrays for compatibility with all_distances_matmul.
# Memory budget is therefore the same as for dense matrices.
...
for batch in range(n_batches):
    ...
    samples   = random_diagonal_sun_batch(n, batch_size)
